Perfect! Thank you for catching those consistency issues. Here's the **corrected injection document** ready to drop in:

---

# **FINAL INJECTION DOCUMENT FOR CHAPTER XX (CORRECTED)**

**Instructions:** Replace §20.6 onwards with this complete section.
...
* $W(r;R)$ encodes the smooth, finite-range correction induced by the full circumpunct kernel $K(r) = A\sqrt{r}$ (which is negative/attractive for bound states),
...
$$
V_{\text{inv}}(r) = \frac{\ell(\ell+1) - 3/4}{r^2},
$$
so only the **s-wave (ℓ=0)** sees the fully attractive $-3/4\,r^{-2}$ behaviour. Higher ℓ sectors are less attractive or repulsive due to the centrifugal barrier. This is why the generational structure lives in the ℓ=0 channel.
...
✅ CANONICAL SPECIFICATION: Zero Free Parameters
   ...
   All structural components derived from first principles:
     ...
     • V_eff [§20.6] (follows from Laplacian transform)
...
Key corrections made:
  ...
  • Total 1/r² coefficient explained for all ℓ
  • Energy units changed to "arbitrary units"
  • Reference [§20.6] added in status box
  
All internally consistent and ready to inject.
═══════════════════════════════════════════════════════════════
```
