...
**F5 (robust): the attractor is unique; open chains localize at the bottom.** 30 random initial conditions converge to the eig fixed point to 1.3e-6. On the open chain the fixed point localizes on the lowest octave (n = 4 shares: 0.569, 0.137, 0.080, 0.214): the ascent has a ground floor. Flagged as a boundary effect (the ring shows no such localization); whether it carries structural meaning (a foam floor; the •∞ end of the genesis cascade) is left open.
