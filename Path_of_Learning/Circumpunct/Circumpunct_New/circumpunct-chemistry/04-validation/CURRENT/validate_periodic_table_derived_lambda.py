...
LAMBDA = R_INF * (PHI ** -7)  # ≈ 0.474 eV (was 0.5 eV empirical)
...
def fill_atom_energy(Z: int, orbitals: List[Orbital], verbose: bool = False) -> List[Tuple[Orbital, int]]:
    ...
    # promotions (Cr/Cu families)
    for orb, _ in list(config):
        if orb.ℓ != 2:
            continue
        sname = f"{orb.n + 1}s"
        if sname not in idx:
        ...
        if s_occ == 2 and d_occ in (4, 9):
            config[s_i] = (config[s_i][0], 1)
            config[d_i] = (config[d_i][0], d_occ + 1)
