...
- **Why this stays at ◐ (not ⊙)**: The primary prediction, A(2) = 10 bp/turn, matches B-DNA in crystal (10.0) but the physiological value in solution is 10.4-10.5 bp/turn (4-5% discrepancy). In physics, our ⊙ derivations achieve sub-ppm accuracy. The crystal value matches exactly, but the framework should predict the physiological form. The discrepancy may reflect a dynamical correction (thermal unwinding shifts the equilibrium geometry), but this correction has not been derived. Additionally: the sugar-phosphate chemistry, why deoxyribose rather than ribose for the stable carrier, and the exact angstrom dimensions are ○-level selections
...
- **Why this stays at ◐ (not ⊙)**: The formal proof that V = 13 is the unique twist-free protofilament count requires showing that the T = 3-start helical lattice closes without supertwist only at V = 1 + T + T² = 13. Empirically, non-13 microtubules develop twist (strong evidence V = 13 is special), but the geometric necessity proof depends on the dimer aspect ratio (~8 nm × ~5 nm), and the width (~5 nm) is not framework-derived. Until the width is either derived or shown to be irrelevant, this remains a four-number pattern match (V, Φ, T, SU(3)) with incomplete uniqueness for V. Additionally: why tubulin specifically (rather than another protein), the GTP/GDP hydrolysis mechanism, and the exact 25 nm outer diameter are ○-level selections
