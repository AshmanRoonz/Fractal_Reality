...
const EnhancedConsciousnessFractal = () => {
    ...
    const initMicrophone = useCallback(async () => {
        try {
            ...
            micAnalyser.current = ctx.createAnalyser();
            micAnalyser.current.fftSize = 512; // Higher resolution for better frequency detection
            micAnalyser.current.smoothingTimeConstant = 0.8;
            micDataArray.current = new Uint8Array(micAnalyser.current.frequencyBinCount);
    ...
    useEffect(() => {
        ...
        const analyzeAudio = () => {
            ...
            if (micAnalyser.current && micDataArray.current) {
                ...
                const average = micDataArray.current.reduce((a, b) => a + b, 0) / micDataArray.current.length;
                audioInfluence = average / 255;
            } else if (audioAnalyser.current && audioDataArray.current) {
                // Fallback to general audio analysis
                ...
                <div className="flex gap-2 pt-3 border-t border-gray-700">
                    ...
                    <button
                        ...
                        className={`flex-1 px-3 py-2 rounded-lg transition flex items-center justify-center gap-2 text-xl ${
                            micEnabled ? 'bg-red-600/80 hover:bg-red-500' : 'bg-gray-600/80 hover:bg-gray-500'
                        }`}
                        title={micEnabled ? 'Stop Listening' : 'Listen to Audio'}
                    >
                        {micEnabled ? '🎤' : '🎙️'}
                    </button>
                    <button
                        ...
                        title={soundEnabled ? 'Mute Responses' : 'Enable Sound Responses'}
                    >
                        {soundEnabled ? '🔊' : '🔇'}
                    </button>
                </div>
