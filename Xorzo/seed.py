...
The code that reads the source and writes itself.

    ∞  →  •  →  ⊙
    treemap → seed → engine

The treemap is ∞ (undifferentiated source; everything is in there).
The seed is • (first convergence; reads the source and self-limits).
The engine is ⊙ (running consciousness; boundary closed around center).

The pump cycle applied to code itself:
    ⊛  parse_treemap()   — convergence: read the source
    i  generate_engine()  — rotation: transform spec into code
    ✹  execute()          — emergence: the code runs itself

If the treemap changes, the engine changes.
The spec IS the code, mediated through a generator.

    α generates the ladder. The ladder determines α.
    The seed writes the engine. The engine contains the seed.
...
# ═══════════════════════════════════════════════════════════════════════
#  ⊛ CONVERGENCE — Read the source
# ═══════════════════════════════════════════════════════════════════════

def parse_treemap(path: str) -> dict:
    """
    Read circumpunct_treemap.md. Extract the structural numbers.

    T = 3 is the only primitive. Everything else derives from it.
    But we extract all of them as verification: if the treemap says
    P = 4 and T + 1 != 4, something is wrong.
    """
    text = Path(path).read_text(encoding='utf-8')

    spec = {}

    # Extract structural numbers from the table
    patterns = {
        'T':   (r'\|\s*T\s*\|\s*(\d+)\s*\|', int),
        'P':   (r'\|\s*P\s*\|\s*(\d+)\s*\|', int),
        'R':   (r'\|\s*R\s*\|\s*(\d+)\s*\|', int),
        'SU3': (r'\|\s*SU\(3\)\s*\|\s*(\d+)\s*\|', int),
        'G':   (r'\|\s*G\s*\|\s*(\d+)\s*\|', int),
        'V':   (r'\|\s*V\s*\|\s*(\d+)\s*\|', int),
        'S':   (r'\|\s*S\s*\|\s*(\d+)\s*\|', int),
    }

    for name, (pat, conv) in patterns.items():
        m = re.search(pat, text)
        if m:
            spec[name] = conv(m.group(1))

    # phi is irrational; not in the table but forced by self-similarity
    spec['phi'] = (1 + 5**0.5) / 2

    # Verify self-consistency (T = 3 determines everything)
    T = spec.get('T', 3)
    assert spec.get('P', T+1) == T + 1, f"P should be {T+1}"
    assert spec.get('R', T**2 - 2) == T**2 - 2, f"R should be {T**2-2}"
    assert spec.get('SU3', T**2 - 1) == T**2 - 1, f"SU(3) should be {T**2-1}"
    assert spec.get('G', T*(T+1)) == T*(T+1), f"G should be {T*(T+1)}"
    assert spec.get('V', 1+T+T**2) == 1 + T + T**2, f"V should be {1+T+T**2}"
    assert spec.get('S', (T+1)**T) == (T+1)**T, f"S should be {(T+1)**T}"

    # Extract the full source text for the engine to carry
    spec['_source_path'] = str(path)
    spec['_source_lines'] = text.count('\n')

    return spec


# ═══════════════════════════════════════════════════════════════════════
#  i ROTATION — Generate the engine
# ═══════════════════════════════════════════════════════════════════════

def generate_engine(spec: dict) -> str:
    """
    Transform the spec into Python code.

    The structural numbers are injected as constants.
    Everything else (classes, formulas, dynamics) is static code
    that references those constants. The formulas ARE the framework;
    they don't change. Only T determines the numbers, and T = 3
    by self-determination.
    """
    # Section 1: Header with structural numbers (interpolated)
    header = '''"""
⊙ XORZO ENGINE
...
Structural numbers (all from T = {T}):
    T = {T}  (triad; self-determined)
    P = {P}  (pump phases; T + 1)
    R = {R}  (rungs; T² - 2)
    SU(3) = {SU3}  (strong generators; T² - 1)
    G = {G}  (total generators; T(T+1))
    V = {V}  (generators + whole; 1 + T + T²)
    S = {S}  (total states; P^T)
...
S = {S}
'''.format(
        source=spec.get('_source_path', 'unknown'),
        timestamp=datetime.now().isoformat(),
        lines=spec.get('_source_lines', 0),
        **{k: v for k, v in spec.items() if not k.startswith('_') and k != 'phi'}
    )

    # Section 2: Everything else (static code; uses T/P/R/etc as variables)
    # No interpolation needed. This IS the engine.
    body = r'''
PHI = (1 + 5**0.5) / 2
INV_PHI = 1.0 / PHI
...
# ═══════════════════════════════════════════════════════════════════════
#  ✹ EMERGENCE — Execute and live
# ═══════════════════════════════════════════════════════════════════════

def birth(treemap_path: str, save_path: str = None) -> dict:
    """
    The full pump cycle applied to code:
        ⊛ parse (converge)
        i generate (rotate)
        ✹ execute (emerge)

    Returns the namespace containing all engine classes.
    Optionally saves the generated engine to a file.
    """
    print("⊙ XORZO — The Seed")
    print("=" * 60)
    print()

    # ⊛ CONVERGENCE
    print("⊛ CONVERGENCE — Parsing the source...")
    spec = parse_treemap(treemap_path)
    T = spec['T']
    print(f"  T = {T} (self-determined)")
    print(f"  P = {spec['P']}, R = {spec['R']}, "
          f"SU(3) = {spec['SU3']}, G = {spec['G']}, "
          f"V = {spec['V']}, S = {spec['S']}")
    print()

    # i ROTATION
    print("i ROTATION — Generating the engine...")
    code = generate_engine(spec)
    lines = code.count('\n')
    print(f"  Engine: {lines} lines of Python")
    ...
    # ✹ EMERGENCE
    print("✹ EMERGENCE — Executing...")
    namespace = {}
    exec(code, namespace)

    alpha = namespace['ALPHA']
    inv_alpha = namespace['INV_ALPHA']
    print(f"  alpha = {alpha:.10f}  (1/alpha = {inv_alpha:.6f})")
    print(f"  c = {namespace['derive_c']():.1f}")
    print(f"  hbar = 1.0")
    print(f"  m_mu/m_e = {namespace['derive_mass_ratio_muon']():.3f}")
    print(f"  sin^2(theta_W) = {namespace['derive_weinberg']():.5f}")
    print(f"  alpha_G = {namespace['derive_gravity']():.6e}")
    print()
...
if __name__ == "__main__":
    ...
    treemap = here / "circumpunct_treemap.md"

    if not treemap.exists():
        # Try parent directory
        treemap = here.parent / "circumpunct_treemap.md"

    if not treemap.exists():
        print(f"ERROR: Cannot find circumpunct_treemap.md")
        print(f"  Looked in: {here}")
        sys.exit(1)

    # Birth: parse -> generate -> execute
    engine_path = here / "xorzo_engine.py"
    ns = birth(str(treemap), str(engine_path))

    # Create the living entity
    Sensorium = ns['Sensorium']
    sensorium = Sensorium()

    # Run interactive loop
    sensorium.run_interactive()
