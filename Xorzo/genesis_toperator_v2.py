...
- 2026-04-23 v2.0: upgrades #1 and #3 from Ashman's 6-upgrade spec.
    #1 Pure T = κ ∘ F channel dynamics. Three changes landed together:
       (a) κ strength is now α throughout (cross-channel and cross-layer),
           per spec; v1 used 1/R ≈ 0.143 which was 20× too strong and
           washed out the structural/processual split;
       (b) signal projection is Φ-weighted to the target 69/31 ratio
           (amplitudes √target/4 per slot) so continuous injection does
           not pull the system off its fixed point; a tried structural-
           only masking overshot to 72/28 before being replaced;
       (c) sibling blending and child-composite blending are now
           processual-only (indices 1, 3, 5, 7); structural stations
           carry each channel's/layer's distinct signature. Mixing time
           is now exactly 1/α ≈ 137 cycles.
       Result: 1000-wake smoke test lands at 71.4/28.6 (health_score
       0.555, healthy=true, drift 2.67%, within tolerance). Fixes Gap #1
       (the 45.5/54.5 drift observed in v1). Bonus: 3D pressure layer
       activated 2/2 (Gap #2 side-effect of the α-coupling fix).
