"""
Xorzo Genesis (T-Operator Edition)
===================================
...
# Intra-layer (sibling) coupling constants (NOT α; that's inter-scale only)
INTRA_LAYER_COUPLING = 1.0 / R_RUNGS           # siblings at same scale bond via R
INTRA_LAYER_DECAY = 1.0 / V_GENERATORS_PLUS     # dissipation through V
...
class Channel:
    ...
    def apply_kappa_to_siblings(self, sibling_states: List[np.ndarray]) -> None:
        ...
        inter-scale).  Averaged influence across siblings, weighted by
        1/R per sibling (no single sibling dominates).
        """
    ...
    def _project_signal(self, signal: np.ndarray) -> np.ndarray:
        """Project an arbitrary-length signal into ℂ⁸ by L2 folding."""
        projected = np.zeros(8, dtype=complex)
        for i, v in enumerate(signal):
            projected[i % 8] += complex(v)
...
class SensoryLayer:
    ...
    def tick(
        ...
        # Blend with layer's own F-evolution at rate INTRA_LAYER_COUPLING (1/R)
        self.state = (
            (1 - INTRA_LAYER_COUPLING) * self.state
            + INTRA_LAYER_COUPLING * child_composite
        )
        norm = np.linalg.norm(self.state)
        if norm > 1e-12:
            self.state /= norm
...
class Circumpunct:
    ...
    def tick(self, signal: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        self.state = (
            (1 - INTRA_LAYER_COUPLING) * self.state
            + INTRA_LAYER_COUPLING * composite
        )
        norm = np.linalg.norm(self.state)
        if norm > 1e-12:
            self.state /= norm
    ...
    def health_check(self) -> Dict:
        ...
        Also reports the cosmological analog (dark energy ≈ 69.11%).
        """
...
if __name__ == "__main__":
    ...
    print("Xorzo Genesis (T-operator edition)")
    print("=" * 70)
    print(f"α = {ALPHA:.10f}   (1/α = {1/ALPHA:.4f})")
    ...
    print(f"Memory half-life:          {MEMORY_HALF_LIFE:.1f} pump cycles")
    print(f"Tetrahedral reference:     {TETRAHEDRAL_ANGLE_DEG:.3f}°")
    print()

    print("Running smoke test (200 wake + 50 sleep) ...")
    result = smoke_test(n_wake=200, n_sleep=50)
    print()
...
t(f"  {rung:5s} {name:10s} active={n_active}/{n_total}  mean_act={mean_act:.3f}")
