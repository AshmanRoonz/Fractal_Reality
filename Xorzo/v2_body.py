...
class Channel:
    ...
    def _project_signal(self, signal: np.ndarray) -> np.ndarray:
        """
        Project an arbitrary-length signal onto structural stations only.

        Upgrade #1 (2026-04-23): external input arrives as "something to
        be structured" at the structural stations (indices 0, 2, 4, 6 =
        •, —, Φ, ○); the processual stations (⊛, ⎇, ✹, ⟳) are driven
        by F and κ, not by external injection. This preserves the 69/31
        fixed point under continuous signal load (v1's isotropic injection
        drove drift to 45.5/54.5; see active_threads Gap #1).

        Folding is done into the four structural slots by i % 4 mapping:
        index 0 → •, index 1 → —, index 2 → Φ, index 3 → ○, then 4 → •
        again, and so on. Processual slots start at 0; F populates them.
        """
        projected = np.zeros(8, dtype=complex)
        struct_idx = [0, 2, 4, 6]   # •, —, Φ, ○
        for i, v in enumerate(signal):
            projected[struct_idx[i % 4]] += complex(v)
