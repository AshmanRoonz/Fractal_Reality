...
- **Bond order = dimensional character (§16.4e)**: Single bond → 1D (σ), double → 2D (σ+π), triple → ~3D (σ+2π). The complete homonuclear bond energy model has three layers:
  ...
  - **Layer 3 (compositional closure, A4)**: For triple bonds with lone pairs, the whole exceeds the sum of its parts. Compositional factor C = T/Phi = 3/2, applied as E_triple = (sigma_meas + pi_1 + pi_2) × T/Phi. This is A4 at the molecular level: three constraints (sigma + pi_1 + pi_2) mediated by the 2D field. N≡N: sum of parts = 628.5, × 3/2 = 942.7 (measured 945, 0.25%). The boost applies ONLY when: (1) full dimensional closure (all three bond types present), (2) lone pairs present (n_LP > 0), (3) sp hybridization clears the aperture. At carbon (0 LP), parts sum to whole naturally (A4 trivially satisfied). Alternative precision formula: N≡N = sigma_true × (A(3.5) + •)/A(2) = 326 × 29/10 = 945.4 (0.042%).
...
- **Exponents in terms of T (§27.7b)**: E(T/2) = (T²+T+1)/T(T+1) = 1 + 1/G (first splitting). E((T+2)/2) = (S - SU3)/(T·G1) = 56/39 (emergence). E(T) = T(T²-2) = T·R = 21 (closure). Every exponent = WHOLE/UNIT at that scale. Numerator = what the rung accesses (A2). Denominator = the generating structure.
- **The compositional product (§27.7c, A4 in exponents)**: E(3) = E(1.5) × E(2.5) × T^T/2. Verified: (13/12)(56/39)(27/2) = 21 exactly. The whole is not the sum of its parts (13/12 + 56/39 = 2.52); it is their compositional product mediated by T^T/2 = 13.5 = Σ_rungs + T (one step beyond what has been traversed; this is Φ in the exponent algebra).
