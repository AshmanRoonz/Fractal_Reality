...
- **Ionic resonance from ◐ deviation (§16.4f)**: Heteronuclear bonds have an additional energy contribution from electronegativity asymmetry. When ◐ ≠ 0.5, the convergence asymmetry stores ionic resonance energy beyond the covalent model. Empirical pi_1 values for C=N (310) and C=O (441) imply sigma_true values 18-45% above the homonuclear arithmetic mean. The amplification scales with delta_EN but is not linear. This is a fourth layer requiring derivation from the balance parameter and is distinct from the three-layer covalent model.
