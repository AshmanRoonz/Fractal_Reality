...
- **Electronegativity = convergence strength (§16.4d)**: EN difference maps to |◐ - 0.5|. Carbon is closest to balance among period 2 bonding elements. Bond polarity is ◐ deviation.
- **Bond order = dimensional character (§16.4e)**: Single bond → 1D (σ), double → 2D (σ+π), triple → ~3D (σ+2π). Energy ratios (C=C/C-C = 1.775, C≡C/C-C = 2.425) fall short of integer ratios due to compounded constraint costs.
