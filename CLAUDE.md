...
- **Electronegativity = convergence strength (§16.4d)**: EN difference maps to |◐ - 0.5|. Carbon is closest to balance among period 2 bonding elements. Bond polarity is ◐ deviation. For heteronuclear bonds, ◐ ≠ 0.5 adds ionic resonance energy (a fourth layer beyond the covalent model; see §16.4f).
...
- **Ionic resonance from ◐ deviation (§16.4f)**: Heteronuclear bonds have an additional energy contribution from electronegativity asymmetry. When ◐ ≠ 0.5, the convergence asymmetry stores ionic resonance energy beyond the covalent model. Empirical pi_1 values for C=N (310) and C=O (441) imply sigma_true values 18-45% above the homonuclear arithmetic mean. The amplification scales with delta_EN but is not linear. This is a fourth layer requiring derivation from the balance parameter and is distinct from the three-layer covalent model. The screening constants (§16.4b2) provide Z_eff from framework principles (3% accuracy), and EN proportional to Z_eff/n works within a period (C, N, O, F within 4%), but the full ionic resonance formula requires both field polarity (◐ deviation) and boundary overlap (atomic size), which a single parameter cannot capture. Deriving the overlap term requires electron affinity from the framework, an open problem.
