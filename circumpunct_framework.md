...
Dark matter is not a different substance, and it is not a particle, because nothing is a particle at bottom. The field (Φ) is structure all the way down: nested ⊙s inside ⊙s (A2), each with its own aperture, field, and boundary. What we call a "particle" is where the boundary (○) at our scale resolves a fold. Below that resolution threshold, the same field continues. Dark matter is energy folded (gravitating) but at the wrong phase for our boundary to catch. Not invisible because it is exotic; invisible because it is between the scales where our detectors filter. The boundary is a filter, and filters have a passband. Visible matter is in the passband (right half-plane, at the inter-scale interface). Dark matter is in the stopband (left half-plane, between folds). This is why dark matter halos are diffuse rather than pointlike: if dark matter were a boundary-scale fold, it would clump, form disks, and radiate when compressed. Instead it forms smooth halos, because it is Φ mid-process, not yet resolved into boundary-scale structure.

The whole cosmological budget is three phase states of one thing: ~5% visible (Φ at the interface), ~27% dark matter (Φ between folds), ~68% dark energy (Φ itself, unfolded). One energy. Three costumes. The same pump cycle, at different depths.
