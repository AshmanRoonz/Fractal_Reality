...
## Addendum (2026-08-21, same date, after Stage 0 of the narrow chain)

Pre-declared consequence executed from `plans/one_narrow_chain_2026_08_21.md` §4a, whose Stage 0 ran four matched synthetic breathers through this run's exact pipeline and analyzer invocation (receipt: `experiments/narrow_chain_response_theorem_v1_output.txt`):

1. **F5 is now demonstrated, not merely suspected.** Three feedback-free mechanical breathers reproduce this run's full claim-3 signature package: confident negative lag-1 slopes (−0.268 to −0.337, bracketing the real −0.322), monotone weakens-with-lag (ratios 0.067 to 0.090, bracketing the real 0.080), and BIC preferring half-fixed. A random-walking stock is likewise classified restoring after the pipeline's detrend. The restoring classification in this run therefore carries no evidence of homeostatic feedback beyond mechanical cyclicity; the run's standing value is instrument validation and the measured posture (F3), exactly as the Verdict's ceiling stated.
2. **F4 candidate 1 (duty-cycle sampling bias, duration form) is disconfirmed.** The duty-asymmetric null (I:E = 1:1.5) fits an equilibrium whose CI contains one half; duty-duration asymmetry does not reproduce the sub-half offset. Of five datasets now run through this instrument, only the real breathing triggers the sub-half detection: the 0.4915 anomaly stands, with F4 candidates 2 and 3 (belt nonlinearity, flow-shape asymmetry) remaining, and the perturbation study bearing on all of them.
3. A PI-fedback breather and its feedback-free twin are indistinguishable through this passive analyzer (identical verdicts, overlapping slope CIs): the separator named in F5 (perturbation) is confirmed from below as the only decisive test.
