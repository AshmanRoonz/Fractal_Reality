...
## The v2 certificate (same day; supersedes the v1 certification chain)

The second model returned an improved certificate, imported, run, and verified to reproduce its claimed numbers exactly: `experiments/finite_window_certificate_v2.py` + `_findings.md`. Three genuine improvements over the v1 chain above, and one correction to this note's own claim:

1. **The transversality lemma (supersedes the Davis-Kahan schematic in Theorem D).** Restricting the IDEAL pair (V V*, V Gamma V*) to ANY 8-dimensional subspace W transverse to ran(V) preserves the generalized eigenvalues exactly: with C = V*W invertible, W* B0* W = C*C and W* B1* W = C* Gamma C, the same congruence on both forms. So movement of the prime-computed signal subspace introduces NO first-order spectral error; it only weakens the metric lower bound through cos^2(theta), with sin(theta) <= eps0/(a - eps0) = 7.6e-6 here. Cleaner than expected, proved in one line.
2. **Fully unconditional zero leakage.** The v2 bounds off-window zeros by an explicit zero-counting theorem (quoted as Bellotti-Wong: |N(T) - (T/2pi) log(T/2pi e)| <= 0.10076 log T + 0.24460 log log T + 8.08344 for T >= e; constants quoted, flagged for source verification) AND allows every tail zero to sit anywhere in the strip (|beta - 1/2| <= 1/2 absorbed by a factor e^(sigma^2/4)). The tail of the certificate does not assume RH. The eight in-window targets are anchored by the reported Platt-Trudgian interval-arithmetic verification (all zeros to height 3e12 on the line), also quoted.
3. **Exact conditioning.** sigma_min(V) = 5.6682, cond(V) = 1.039: the packet family is nearly orthonormal on the targets; a = 32.1289 as computed before.

**Correction to this note's v1 certification (visible, not silent).** The v1 chain's C3 step used the ACTUAL leakage computed from known zeros (2.1e-8) inside a bound it described as unconditional. That was an overstatement: known-zero leakage is semi-empirical. The fully unconditional certified radius is the v2 number:

    |gamma_hat_r - gamma_r| <= 6.9676e-4   for each of the eight modes,

with 2 delta = 1.39e-3 against a minimum target gap of 2.4084: **unique mode matching certified with a margin of about 1,700x.** The v1 figure of 1.5e-4 is retracted as a headline (it remains a valid semi-empirical estimate); 7e-4 is the theorem. Error budget of the v2 certificate: prime tail eps0 <= 1.97e-4 (Lambda(n) <= log n, integral comparison, no oscillation credit), zero tail eps0 <= 4.73e-5 (unconditional, strip-wide), poles < 1e-94; totals eps0 <= 2.44e-4, eps1 <= 1.18e-2. The observed accuracy remains ~1e-12; everything between 7e-4 and 1e-12 is oscillatory cancellation the theorem refuses to spend.

**The finite theorem, final form:** a finite prime-built Weil metric, paired with the scale derivative, reconstructs the eight target spectral ordinates with an explicit analytic error bound below 7e-4, uniquely matched to their targets, while self-adjointness makes their reality structural. Nothing controls the window-to-infinity limit; that remains RH.
