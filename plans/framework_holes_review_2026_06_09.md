...
**RESOLVED (Ashman, 2026-07-29): the third option, exactly as stated above.** The sequence is re-derived from the circuit and runs in two directions: formation ascends TRUE → FAITHFUL → GOOD → RIGHT → AGREEMENT (the four beats with the virtues as structural partners; each station launches the process at d + 0.5, so GOOD launches ✹/LETTING and RIGHT launches ⟳/CHECKING, per this section's stated derivation rule); validation walks it in reverse (audit form AGREEMENT → RIGHT → GOOD → FAITHFUL → TRUE; earning form RIGHT → GOOD → FAITHFUL → TRUE → AGREEMENT). The old sequence is retired as an old-ladder artifact; its claims distribute between the directions. Files: framework §25.11 (restructured), §25.18b (corrected table, corrected reading keys), §25.19 (bearing note resolved), CLAUDE.md (open decision 1 closed), the book's summary lists, `Circumpunct_ToE.md`, `circumpunct_treemap.md`, `docs/the_staggered_octave.html` §6 status note. This section predicted the resolution verbatim in June; the derivation-rule discipline it demanded (state the rule, accept what falls out) is what the adjudication followed.
